...
@section('content')
    ...
    <form class="d-flex justify-content-center align-items-center mb-4" id="" method="get" action="{{url('/')}}">
        @csrf
        <button type="submit"  class="btn btn-outline-info btn-md ms-2">Show Pending Task</button>
    </form>
                        ...
                        <table class="table text-white mb-0">
                          ...
                          <tbody >
                           ...
                           <tr class="dataTask{{$task->id}}"id="datanew">
                            ...
                            <td>
                              @if($task->isComplete)
                              <button class="btn btn-success btn-sm text-white font-weight-bold">Completed...<i class="fa fa-check"></i></button>
                              @else
                              <button class="btn btn-warning btn-sm text-white font-weight-bold">Pending...<i class="fa fa-exclamation"></i></button>
                              @endif
                            </td>
                            <td>
                                <a class="deletebtn btn" data-id="{{ $task->id }}" ><i class="fa fa-trash" style="color:red;"></i></a></td>
                           </tr>                       
                           @endforeach                           
                          </tbody>
                        </table>
